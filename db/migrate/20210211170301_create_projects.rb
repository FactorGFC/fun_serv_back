class CreateProjects < ActiveRecord::Migration[6.0]
  def change
    create_table :projects, id: :uuid do |t|
      t.string :project_type, null: false
      t.string :folio, null: false
      t.decimal :client_rate, null: false, precision: 15, scale: 4
      t.decimal :funder_rate, null: false, precision: 15, scale: 4
      t.decimal :ext_rate, null: false, precision: 15, scale: 4
      t.decimal :total, null: false, precision: 15, scale: 4
      t.decimal :interests, null: false, precision: 15, scale: 4
      t.decimal :financial_cost, null: false, precision: 15, scale: 4
      t.string :currency, null: false
      t.date :entry_date, null: false
      t.date :used_date, null: false
      t.string :status, null: false
      t.string :attached
      t.string :extra1
      t.string :extra2
      t.string :extra3
      t.references :customer, type: :uuid, null: false, foreign_key: true
      t.references :user, type: :uuid, null: false, foreign_key: true
      t.references :project_request, type: :uuid, null: false, foreign_key: true
      t.references :term, type: :uuid, null: false, foreign_key: true
      t.references :payment_period, type: :uuid, null: false, foreign_key: true
      t.references :credit_rating, type: :uuid, null: false, foreign_key: true

      t.timestamps
    end
  end
end
