...
class Api::V1::CustomerCreditsController < Api::V1::MasterApiController
      ...
      @customer_credit.end_date = @end_date
        if @customer_credit.save
          ...
          if @customer_credit.status == 'SI'
              ...
              raise ActiveRecord::Rollback
          elsif @customer_credit.status == 'PROPUESTO'
             #TO DO: AGREGAR MAILER CON CALLBACK PARA LA CONFIRMACION DEL CLIENTE DEL NUEVO CREDITO CON ESTATUS PROPUESTO
  ...
  def generate_token
    return if token.present? && token_expiry > Time.now
    self.token = token
