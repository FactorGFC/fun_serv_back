class ApidocsController < ApplicationController
  ...
  swagger_root do
    ...
    info do
      key :version, '1.0.0'
      key :title, 'Funder_backend API'
      key :description, 'Backend para sistema de fondeo colectivo'
      contact do
        key :name, 'Eloy Rodríguez'
        key :email, 'eloyra@gmail.com'
      end
    end
    tag do
      key :name, 'User'
    ...
    tag do
      key :name, 'Funder'
      key :description, 'Operaciones para inversionistas | SP: funders'
    end
    tag do
      key :name, 'ExtService'
    ...
    tag do
      key :name, 'ProjectRequest'
      key :description, 'Operaciones con solicitudes de proyecto | SP: project_request'
    end
    tag do
      key :name, 'Project'
      key :description, 'Operaciones con proyectos | SP: project'
    end
    tag do
    ...
    tag do
      key :name, 'FundingRequest'
      key :description, 'Operaciones con solicitudes de fondeo| SP: funding_request'
    end
    tag do
      key :name, 'Investment'
      key :description, 'Operaciones con inversiones| SP: investment'
    end
    tag do
      key :name, 'SimFunderYield'
      key :description, 'Operaciones con rendimientos de inversiones| SP: sim_funder_yield'
    end
    tag do
  ...
  SWAGGERED_CLASSES = [
    ...
    Rate,
    Api::V1::ProjectRequestsController,
    ProjectRequest,
    Api::V1::ProjectsController,
    Project,
    Api::V1::CustomerCreditsController,
    CustomerCredit,
    Api::V1::FundingRequestsController,
    FundingRequest,
    Api::V1::InvestmentsController,
    Investment,
    Api::V1::SimFunderYieldsController,
    SimFunderYield,
